...
function showWords(dataMuseURL) {
    datamuseRequest(dataMuseURL(wordInput.value), (result) => {
        let syllablesResult = groupBy(result, 'numSyllables');
        if (result.length == 0) {
            ...
            if (dataMuseURL == getDatamuseRhymeUrl) {
                for (let key of Object.keys(syllablesResult)) {

                    let syllablesNum = document.createElement('h3');
                    syllablesNum.innerHTML = `Syllables: ${key}`;
                    wordOutput.appendChild(syllablesNum);
                    
                    for (let word = 0; word < syllablesResult[key].length; word++) {
                        let item = document.createElement('li');
                        item.innerHTML = `\n                    <span class="item">${syllablesResult[key][word].word}</span>                   \n                    <button class="btn btn-sm btn-outline-success save" type="button" id=${syllablesResult[key][word].word}> (save) </button>`;
                        wordOutput.appendChild(item);
                    } 
                }
            }
                ...
                for (let word = 0; word < result.length; word++) {
                    let item = document.createElement('li');
                    item.innerHTML = `\n                    <span class="item">${result[word].word}</span>                   \n                    <button class="btn btn-sm btn-outline-success save" type="button" id=${result[word].word}> (save) </button>`;
                    wordOutput.appendChild(item);
                } 
            }

            var saveButtons = document.querySelectorAll("li .btn");
            for (var i = 0; i < saveButtons.length; i++) {
                saveButtons[i].addEventListener("click", function(e) {
                    var el = e.target;
                    addToSavedWords(" " + el.id);
                });
            };
        };
    });
...
showSynonymsButton.addEventListener('click', function() {
    ...
    showWords(getDatamuseSimilarToUrl);
});
